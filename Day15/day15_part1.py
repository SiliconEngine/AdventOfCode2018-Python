...
def move_unit(grid, units, targets, unit):
    ...
    # Step 1: For unit, identify list of points in range around each
    in_range = set()
    ...
    # Step 2: Figure out which are reachable
    reachable = []
    ...
    # Step 3: Figure out nearest reachable nodes
    min_len = min([ distances[r] for r in reachable ])
    nearest = [ r for r in reachable if distances[r] == min_len ]

    # Step 4: Choose destination in "reading order"
    nearest.sort(key=lambda item: (item[1], item[0]))
    dest = nearest[0]

    # Step 5: Figure out first steps of paths to nearest and choose first in "reading order"
...
def main():
    ...
    #dsp(grid, start_units)

    cycle = 0
    units = start_units.copy()
    while True:
        cycle += 1
        ...
        for unit in units:
            ...
            # Identify enemy targets and see if any in range to attack
            enemy = 'G' if unit.type == 'E' else 'E'

            targets = { (u.x, u.y): u for u in units if u.type == enemy }
            if len(targets) == 0:
                return cycle-1, units       # No more enemies, so we're done

            # See if any enemies are right next to us
            x, y = unit.x, unit.y
            attack_list = []
            for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
            ...
            # Nothing to attack, so process move
            if not attack_list:
